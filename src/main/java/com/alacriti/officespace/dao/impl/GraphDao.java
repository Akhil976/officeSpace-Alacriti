...
public class GraphDao {
	public static GraphWeekVo getWeekUsage(Connection connection,GraphAnalyticsRequestVo graphAnalyticsRequest) {
		...
		try {
			...
			for (int i = 0; i < 7; i++) {
				System.out.println("select sum(quantity) from officespace_items_consume_tbl where item_id="+itemId+" and consume_date like '"+calenderValues[0]+"-"+new DecimalFormat("00").format(month)+"-"+new DecimalFormat("00").format((weekStartDate+i))+"'");
	...
	public static GraphMonthVo getMonthUsage(Connection connection,GraphAnalyticsRequestVo graphAnalyticsRequest) {
		...
		try {
			st = connection.createStatement();
			for (int i = 1; i < 31; i++) {
				System.out.println("select sum(quantity) from officespace_items_consume_tbl where item_id="+itemId+" and consume_date like '"+calenderValue+"-"+new DecimalFormat("00").format(i)+"'");
