...
public class AnalyticsDao {
	...
	public static int getWeekUsage(Connection connection,AnalyticsRequestVo analyticsCustom) {
		...
		System.out.println(calenderValue);
		String[] calenderValues = calenderValue.split("-");
		System.out.println(calenderValues[0]+"calenderValues: "+calenderValues[1]);
		try {
			st = connection.createStatement();
	...
	public static PaginationConsumeVo getPagination(Connection connection, PaginationRequestVo paginationRequestVo){
		...
		try {
			...
			for(int i=1; i<31; i++){
				...
				System.out.println("insert into officespace_pagination_consume values("+itemId+","+categoryId+","+calenderValue+","+consumeQuantity+","+uniqueId+","+i+")");
				st.executeUpdate("insert into officespace_pagination_consume values("+itemId+","+categoryId+",now(),"+consumeQuantity+","+uniqueId+","+i+")");
			...
			uniqueId++;
			System.out.println("before calling delegate");
